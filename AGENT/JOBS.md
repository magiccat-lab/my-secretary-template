...
| トリガー | スクリプト | タグ | 動作 |
|---------|--------|------|--------|
| `cron: 0 9 * * MON` | `scripts/weekly_news.py` | `core` | 週次ニュースを #random に投稿 |
| キーワード "log mood" | 会話 | `core` | `data/mood.md` に追記 |

**タグ** は複数秘書モードでのルーティングに使います。`secretaries.yaml` の
`jobs.enabled_tags` でどの秘書がどのタグのジョブを担当するか制御します。
1体運用なら全部 `core` で OK。
