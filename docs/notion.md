...
エージェントがユーザーの「〇〇って店行きたい」「〇〇おすすめらしい」を受けたら CLI を叩く:
...
オプション:
- `--genre`: ジャンル（和食 / イタリアン / フレンチ / 中華 / 焼肉 / 寿司 / カフェ / バー / その他）
- `--budget`: 予算帯（~3000円 / 3000~5000円 / 5000~10000円 / 10000円~）
- `--private-room`: 個室あり（フラグ指定）
- `--url`: URL
- `--status`: ステータス（行きたい / 行った / お気に入り、デフォルト: 行きたい）

---

## 4.5 お店リスト DB スキーマ

> 💡 テンプレート複製で設定済み。手作業でプロパティを作る必要はありません。

| プロパティ | 型 | 役割 |
|---|---|---|
| `店名` | title | お店の名前 |
| `ジャンル` | select | 和食 / イタリアン / フレンチ / 中華 / 焼肉 / 寿司 / カフェ / バー / その他 |
| `エリア` | rich_text | エリア（自由入力） |
| `予算帯` | select | ~3000円 / 3000~5000円 / 5000~10000円 / 10000円~ |
| `個室` | checkbox | 個室の有無 |
| `URL` | url | お店の URL（食べログ / Google Maps 等） |
| `メモ` | rich_text | 備考 |
